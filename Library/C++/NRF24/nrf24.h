...
// Địa chỉ giả để test truyền nhận
...
enum
{
    ...
    nRF24_ARD_NONE = (uint8_t)0x00, // Khong sử dụng retransmit
...
enum
{
    ...
    nRF24_DR_250kbps = (uint8_t)0x20, // 250kbps
    nRF24_DR_1Mbps = (uint8_t)0x00,   // 1Mbps
    nRF24_DR_2Mbps = (uint8_t)0x08    // 2Mbps
};
...
// Địa chỉ Pipe address và địa chỉ TX address
enum
{
    ...
    nRF24_PIPETX = (uint8_t)0x06 // TX address
};

// trạng thái auto acknowledgment cho công chỉ định
enum
{
...
enum
{
    ...
    nRF24_STATUS_RXFIFO_DATA = (uint8_t)0x00,  // 	RX FIFO chứa data vẫn còn chỗ trống
    nRF24_STATUS_RXFIFO_EMPTY = (uint8_t)0x01, //	RX FIFO trống
    nRF24_STATUS_RXFIFO_FULL = (uint8_t)0x02,  //	RX FIFO đã đầy
    nRF24_STATUS_RXFIFO_ERROR = (uint8_t)0x03  //	RX FIFO đéo thể trống và tràn tại cùng 1 thười điểm
};

// trạng thái của TX FIFO
enum
{

    nRF24_STATUS_TXFIFO_DATA = (uint8_t)0x00,  // TX FIFO chứa data vẫn còn chỗ trốngs
    nRF24_STATUS_TXFIFO_EMPTY = (uint8_t)0x01, // TX FIFO trống
    nRF24_STATUS_TXFIFO_FULL = (uint8_t)0x02,  // TX FIFO đã đầy
    nRF24_STATUS_TXFIFO_ERROR = (uint8_t)0x03  // TX FIFO đéo thể trống và tràn tại cùng 1 thười điểm
};
...
typedef enum
{
    ...
    nRF24_RX_PIPE0 = (uint8_t)0x00, // Packet được nhận từ PIPE#0
    nRF24_RX_PIPE1 = (uint8_t)0x01, // Packet được nhận từ PIPE#1
    nRF24_RX_PIPE2 = (uint8_t)0x02, // Packet được nhận từ PIPE#2
    nRF24_RX_PIPE3 = (uint8_t)0x03, // Packet được nhận từ PIPE#3
    nRF24_RX_PIPE4 = (uint8_t)0x04, // PPacket được nhận từ PIPE#4
    nRF24_RX_PIPE5 = (uint8_t)0x05, // Packet được nhận từ PIPE#5
    nRF24_RX_EMPTY = (uint8_t)0xff  //	RX FIFO Trống
} nRF24_RXResult;

// địa chỉ của RX_PW_P# registers
...
// Địa chỉ của các Pipe và TX address registers
